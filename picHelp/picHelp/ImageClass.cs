...
namespace picHelp
{
    public class ImageClass
    {
         ...
         //   System.Drawing.Image bitmap2 = new System.Drawing.Bitmap(towidth, toheight);
            ...
            //    FileExt = Path.GetFileNameWithoutExtension(originalImagePath);
             ...
             //   draw = Graphics.FromImage(bitmap2);
                ...
                bitmap.Save(thumbnailPath + Path.GetFileNameWithoutExtension(originalImagePath) + ".jpg", System.Drawing.Imaging.ImageFormat.Jpeg);
        ...
        public static string LetterWatermark(string path, int size, string letter, Color color, string location)
        {
            ...
            string kz_name = Path.GetExtension(path);
            if (kz_name == ".jpg" || kz_name == ".bmp" || kz_name == ".jpeg"|| kz_name==".gif")
            {
                DateTime time = DateTime.Now;
                string filename = "" + time.Year.ToString() + time.Month.ToString() + time.Day.ToString() + time.Hour.ToString() + time.Minute.ToString() + time.Second.ToString() + time.Millisecond.ToString();
                Image img = Bitmap.FromFile(path);
        ...
        private static ArrayList GetLocation(string location, Image img, int width, int height)
        {
            ...
            else if (location == "中")//中上T
            {
                x = img.Width / 2 - (width * height) / 2;
                loca.Add(x);
                loca.Add(y);
            }
            else if (location == "右上")//右上RT
            {
                x = img.Width - width * height;
            }
            else if (location == "左")//左中LC
            {
                y = img.Height / 2;
            }
            else if (location == "中间")//居中C
            {
                x = img.Width / 2 - (width * height) / 2;
                y = img.Height / 2;
            }
            else if (location == "右")//预定位置右中//出现严重问题无法显示需改进计算RC
            {
                x = img.Width - width * height;

             //   x = img.Width - height;//导致严重问题语句
                y = img.Height / 2;
            }
            else if (location == "左下")//左下LB
            {
                y = img.Height - width - 5;
            }
            else if (location == "下")//中下B
            {
                x = img.Width / 2 - (width * height) / 2;
                y = img.Height - width - 5;
            }
            else if(location=="右下")//右下
            {
                x = img.Width - width * height;
                y = img.Height - width - 5;
            }
